########################################################################################################
# The RWKV Language Model - https://github.com/BlinkDL/RWKV-LM. VERSION v3. Modified for DGA detection.
########################################################################################################
...
T_MAX = 256          # increase this if your ctx_len > 1024
B_GROUP_FORWARD = 1   # set to 8 for best performance
B_GROUP_BACKWARD = 1  # set to 2 for best performance (sometimes 8 is faster)

timex_cuda = load(name="timex", sources=["rwkv_src/cuda/timex_op.cpp", "rwkv_src/cuda/timex_cuda.cu"],
                  verbose=True, extra_cuda_cflags=['--use_fast_math', f'-DTmax={T_MAX}', f'-DBF={B_GROUP_FORWARD}', f'-DBB={B_GROUP_BACKWARD}'])
...
class GPTConfig:
    def __init__(self, vocab_size, ctx_len,classes, class_weights=None, **kwargs):
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.head = nn.Linear(config.n_embd, config.classes, bias=False)
    ...
    def forward(self, idx, targets=None):
        ...
        x = x[:,-1,:] # Only use last state for predicition

        x = self.ln_out(x)
        x = self.head(x)
        if targets is not None:
            if self.config.classes == 1:
                loss = F.binary_cross_entropy_with_logits(x.view(-1, x.size(-1))[:,0].float(), targets.view(-1).float())
            else:
                # Only use class weight loss for training (to match the TF implentation)
                # x = F.softmax(x, dim=1) # Cross-Entropy does LogSoftmax already!!!
                loss = F.cross_entropy(x, targets, weight=self.config.class_weights)
